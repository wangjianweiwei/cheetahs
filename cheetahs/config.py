...
from cheetahs.errors import ConfigError
from cheetahs.reloader import reloader_engines
from cheetahs import __version__, util
...
class Config(object):
    ...
    def __getattr__(self, item):
        if item not in self.settings:
            raise AttributeError(f"没有为{item}设置配置")

        return self.settings[item].get()
    ...
    def set(self, name, val):
        ...
        if name not in self.settings:
            raise AttributeError(f"没有为{name}设置配置")
        self.settings[name].set(val)
    ...
    def worker_class_str(self):
        return self.settings['worker_class'].get()
...
class SettingMeta(type):
    def __new__(mcs, name, base, attrs):
        ...
        parents = [b for b in base if isinstance(b, SettingMeta)]
        # 如果不是继承自`SettingMeta`
        if not parents:
            return super_new(mcs, name, base, attrs)

        attrs["order"] = len(KNOWN_SETTINGS)
        attrs["validator"] = staticmethod(attrs['validator'])
        new_class = super_new(mcs, name, base, attrs)
...
class Setting(object):
    ...
    def __repr__(self):
        return f"<{self.__class__.__module__}.{self.__class__.__name__} object at {id(self)} with value {self.value}>"
    ...
    def set(self, val):
        if not callable(self.validator):
            raise TypeError(f"无效的validator: {self.name}")
...
def validate_pos_int(val):
    ...
    if val < 0:
        raise ValueError(f"val必须是正数: {val}")
...
def validate_bool(val):
    ...
    if val.lower() == "true":
        ...
        raise ValueError(f"无效的bool: {val}")
